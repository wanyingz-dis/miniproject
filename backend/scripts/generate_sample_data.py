    ...
    for exp_id in experiments_df['id']:
        ...
        for _ in range(n_trials):
            ...
            accuracy = random.uniform(
                0.3, 0.95) if status == 'finished' else None
            duration = random.randint(1000, 100000) if status in [
            ...
            data.append({
                ...
                'status': status,
                'created_at': (datetime.now() - timedelta(hours=random.randint(1, 500))).strftime('%d/%m/%Y %H:%M'),
                'accuracy': accuracy,
                'duration(s)': duration
            })
            trial_id += 1
    ...
    for trial_id in trials_df['id']:
        ...
        for _ in range(n_runs):
            ...
            data.append({
                ...
                'created_at': (datetime.now() - timedelta(hours=random.randint(1, 400))).strftime('%d/%m/%Y %H:%M')
            })
            run_id += 1
...
def main():
    ...
    avg_accuracy = trials_df[trials_df['status']
